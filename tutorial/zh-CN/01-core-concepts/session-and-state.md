...
:::note
本章要回答的核心问题：**下一次请求应该相信什么？** 答案只有一个来源——权威日志。
:::
...
:::tip
Session 不是「聊天记录数组」，而是**有明确写入协议的状态容器**。三种视角只能有一种权威来源。
:::
...
:::note
两条不变量的核心：**单一事实来源** + **未闭合不恢复**。后续所有章节都建立在这个基础上。
:::
...
:::caution
关键不是"存下来"，而是区分**哪种存储是权威，哪种存储只是视图**。投影可以重建，权威日志不能。
:::
...
:::danger
身份字段如果只放在进程内存里，重启后会全部失真。子 Agent 的深度尤其典型：如果只在运行时计数，恢复后一个本该受限的孩子会被误当成顶层任务。
:::
