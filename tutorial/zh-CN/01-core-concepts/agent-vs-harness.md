...
review:
  polish:
    ...
    date: 2026-08-23
    verdict: pass
    summary: 已按学习契约重组双读者结构，完成第二遍语言润色，并按 v0.3 九层法补齐定义、参数环境、失败恢复与迁移检查单。
  implementation:
    agent: main-agent
    date: 2026-08-23
    verdict: pass
...
:::note
这三个问题分别指向**状态**、**控制面**和**审计**。本章先把职责边界讲清楚；后面讨论 Run 生命周期时，才能准确回答状态由谁迁移、事件由谁发布、副作用由谁审批。
:::
...
:::tip
理想设计不是给三层各起几个目录，而是把三种能力拆开：**有人负责决定，有人负责约束和记录，有人提供真实资源。**
:::
...
| 角色 | 类比 | 职责 |
| --- | --- | --- |
| Agent | 实验设计师 | 根据目标和已有证据，提出下一步该做什么实验 |
| Harness | 实验室管理员 | 确认申请、准备材料、检查防护、记录过程，再把结果交回设计师 |
| Runtime | 实验室设施 | 提供水电、通风和仪器，但并不关心这次实验想证明什么 |
...
这个类比也有失效边界：真实实验室管理员通常能看到整个房间，而软件里的 Harness 只能约束它真正接入的执行路径。某个工具如果自带网络客户端、绕过统一分发器，管理员就「看不见」这次访问。所以类比只负责建立第一层直觉，接下来仍要回到字段、状态和控制流。

## 机制深拆

### 决策输入与动作输出

先精确定义三个词。**Agent** 在本书中指拥有任务目标、根据观察产生下一步意图的决策体；它通常包含模型，但也可以叠加策略、记忆和子任务组织。**Harness** 是把这些意图变成受控执行的控制面；**Runtime** 是最终提供进程、文件和网络等真实能力的承载环境。
...
只有 Harness 校验参数、拿到授权并交给 Runtime 后，这次「写入」才从意图变成真正发生的副作用。
:::

工具结果随后还要写回权威日志；下一轮模型看到的，才是已经发生的观察，而不是它自己刚才的想象。
...
这条链也会随宿主环境改变。本地 CLI 的「审批」可能只是终端确认；桌面应用要考虑窗口生命周期和系统权限；服务端则必须处理请求身份、租户隔离、审计存储和并发竞争。环境越开放，控制面要补的环节越多，但六个阶段的顺序不变。

### 反例与故障模式

**反例 1：把模型客户端当 Agent。**

程序直接把用户文本发给模型，再把回复展示出来。它能对话，却没有权威日志，也没有副作用边界。

:::danger
一旦加入「帮我删掉临时文件」这样的工具，模型输出的删除意图很容易被当成命令执行；如果执行被拒绝，拒绝原因也不会回到模型可见的观察里，下一轮它可能继续提出同样的危险请求。
:::

**反例 2：按包名判断安全边界。**

某个模块叫 `runtime`，维护者便默认它天然负责隔离。可如果文件路径其实由上层拼接、网络客户端由工具层创建，这个名字挡不住任何越权访问。

判断边界时，与其看名字，不如追问四个事实：
...
进程崩溃后，维护者从磁盘里找回一段 Session 日志，便直接把最后几条消息重新发给模型。这个做法看起来能续跑，实际上可能重复执行未闭合的副作用：文件已经被写入一半，外部任务已经创建，但日志里没有记录这些动作是否完成。恢复语义属于后续 Checkpoint 和 Persistence 章节；本章只需记住，没有权威状态所有者的「恢复」，只是把事故重演一遍。
...
修复方式也不是「把所有错误都打印得更详细」。Harness 需要同时做两件事：把越界错误放进模型可见观察，让 Agent 能调整下一步；把同一条事实追加到权威日志，让用户界面、审计系统和后续恢复看到一致结果。
...
如果你要从零实现一个最小 Harness，迁移路径可以按风险逐步推进：

1. 先让所有工具调用经过同一个入口，即使这个入口暂时只做日志。
2. 再加入参数校验和统一错误结构，让模型能看到可修正的失败原因。
3. 然后区分只读、写入、网络和进程执行，给高危动作接入审批或默认拒绝。
4. 最后引入权威日志、事件投影和恢复检查。

不要从「先把所有权限系统设计完美」开始；也不要长期停留在第 1 步。每前进一格，都应让一条原本不可审计的路径变得可观察。
