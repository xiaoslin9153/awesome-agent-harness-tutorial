...
| 阶段 | 名称 | 目标 | 状态 | 完成标准 |
| --- | --- | --- | --- | --- |
| P0 | 基础设施与治理 | 建立 Git、推送、记录、对比和语言接口。 | `进行中` | 核心治理完成；自动化检查未建。 |
| P1 | 知识基础 | 建立概念总纲、术语表和第一篇核心教材。 | `未开始` | 中文总览与 Agent Run 生命周期发布为 draft。 |
...
| P6 | GitHub Pages 发布 | 选择生成器并建立部署流水线。 | `未开始` | 站点构建成功且公开页面可访问。 |
...
| ID | 任务 | 状态 | 验收标准 | 最近更新 |
| --- | --- | --- | --- | --- |
| K01 | 创建中文教材总览 | `未开始` | `tutorial/zh-CN/00-overview.md` 符合 Front Matter 规范。 | 2026-08-22 |
| K02 | 定义读者路径和学习地图 | `未开始` | 新读者知道从哪里开始、学到什么程度。 | 2026-08-22 |
| K03 | 撰写 Agent Run 生命周期 | `未开始` | 覆盖输入到 final answer、trace 和 resume 的主线。 | 2026-08-22 |
...
| ID | 任务 | 状态 | 说明 | 最近更新 |
| --- | --- | --- | --- | --- |
| F00 | 建立 C01-C25 证据索引模板 | `未开始` | 每条发现绑定仓库、commit、文件路径和行号。 | 2026-08-22 |

### Reasonix

| ID | 任务 | 状态 | 锚点 | 最近更新 |
| --- | --- | --- | --- | --- |
| FR01 | 架构与入口盘点 | `未开始` | `aa82b2f` | 2026-08-22 |
...
| ID | 任务 | 状态 | 锚点 | 最近更新 |
| --- | --- | --- | --- | --- |
| FD01 | Monorepo 与协议盘点 | `未开始` | `b150a55` | 2026-08-22 |
...
| ID | 任务 | 状态 | 锚点 | 最近更新 |
| --- | --- | --- | --- | --- |
| FP01 | Agent / AI / Protocol 盘点 | `未开始` | `c49906e` | 2026-08-22 |
...
| S02 | 初始化最小站点工程 | `进行中` | Node 构建器读取 `tutorial/`，不侵入内容结构。 | 2026-08-22 |
| S03 | GitHub Actions 构建 | `进行中` | 已添加 Pages workflow，待远端构建验证。 | 2026-08-22 |
| S04 | GitHub Pages 部署 | `进行中` | 已配置 workflow，待公开 URL 验证。 | 2026-08-22 |
| S05 | 部署后页面检查 | `未开始` | 检查入口、路由、代码块、表格和链接。 | 2026-08-22 |
| S06 | 自定义域名（可选） | `未开始` | 如启用，需验证 HTTPS 和回退行为。 | 2026-08-22 |

## 当前优先级队列

1. K01：创建 `tutorial/zh-CN/00-overview.md`。
2. F00：建立 C01-C25 证据索引模板。
3. FR02 / FD02 / FP02：横向定位三家 Run 生命周期。
4. FR03 / FD03 / FP03：横向定位三家状态模型。
5. S01：在第一篇教材稳定后做站点选型。
