...
| S04 | GitHub Pages 部署 | `进行中` | Workflow 已配置部署；本会话仍需对最新 commit 执行线上验证。 | 2026-08-22 |
| S05 | 部署后页面检查 | `进行中` | 已有部署检查协议；每次推送后检查入口和受影响路由。 | 2026-08-22 |
| S06 | 自定义域名（可选） | `未开始` | 如启用，需验证 HTTPS 和回退行为。 | 2026-08-22 |

## 当前优先级队列

1. 完成 C03/C04，补齐第一章。
2. 按 TOC 顺序批量撰写 M-01 至 M-16，保留源码锚点与 `pending_review` 清单。
3. 补齐 C01-C25 证据索引，支撑框架深拆。
4. 完成三家框架架构、生命周期和工具安全章节。
5. 全部初稿后统一 Implementation Review，再进入最终发布检查。
