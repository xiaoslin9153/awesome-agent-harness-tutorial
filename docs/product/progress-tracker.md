...
| G24 | 固化 Goal 串行执行与限流恢复 | `已完成` | 任一时刻最多一个 Subagent；429 后指数退避并从原阶段恢复。 | 2026-08-22 |
| G25 | 固化 402 支付失败处理 | `已完成` | 402 不盲目重试；检查余额与输出预算后降低 max tokens、补足额度或切换模型。 | 2026-08-22 |
...
2. 每节串行执行 Draft → Polish → 自检 → 提交推送 → Deploy 检查 → 进度同步；任一时刻最多一个 Subagent。
3. 补齐 C01-C25 证据索引后，再按同一节奏完成三家框架深拆。
4. 全部初稿完成后统一 Implementation Review，最后进入发布检查。
